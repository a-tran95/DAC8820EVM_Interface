...
V = A(:,5);
T = (A(:,4)+abs(A(1,2)));
vlevels8 = [0; 1.2; 2.6 ; 3.8; 5; 6.4; 7.6; 8.8];       % Voltage Levels (PAM-8)
blevels = [0; 1; 2; 3; 4; 5; 6; 7];                     % Bit Level
...
for i = 1:length(V_sample)
    decide = vlevels8 - V_sample(i);
    [decval,decidx] = min(abs(decide));
    V_level(i) = vlevels8(decidx);
end

% Determining corresponding bit level
for m = 1:length(V_level)
    B_level(m) = blevels(find(vlevels8 == V_level(m)));
end 
